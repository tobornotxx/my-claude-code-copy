...
**两条召回路径分工明确：** Path A 是静态注入——系统说明加 MEMORY.md 索引，粗粒度、零成本、始终可用；Path B 是语义召回——异步预取，用 Sonnet 从 frontmatter 清单中选出最多 5 个最相关的记忆文件注入，成本更高但精度也更高。

注意：记忆系统和**项目指令系统（CLAUDE.md）是两套独立的机制**。CLAUDE.md 是人工维护的项目规则文件（checked into git），通过四层层级加载（Managed → User → Project → Local）；而记忆系统是 Agent 自动提取的跨会话知识（存在 `~/.claude/projects/<hash>/memory/`）。二者共享同一个注入入口（getUserContext），但来源、维护方式和生命周期完全不同。CLAUDE.md 还有一个嵌套延迟加载机制——只有模型读到某个目录下的文件时才加载该目录的 CLAUDE.md 指令文件，这属于项目指令系统的设计，不是记忆召回。
...
**附件系统的分时注入。** 不同上下文信息在不同时机注入。system prompt 和 MEMORY.md 索引在循环开始时注入，作为稳定前缀保护 cache key；压缩在 API 调用前完成；记忆预取和技能发现预取都在循环入口异步启动（基于用户输入和上一轮的消息历史），在 API 调用和工具执行期间并行完成，工具执行后零等待消费——这是纯粹的并行优化。而 CLAUDE.md 嵌套加载和文件变更附件则必须在工具执行后注入，因为它们依赖模型这一轮的行为：只有模型读了某个目录的文件，才知道要加载哪个 CLAUDE.md；只有工具跑完了，才知道改了哪些文件。
